...
I identify seven features that are common to most of the models but differ in details according to the model parameters. They are ``location of the termination'', ``geometry of the trough'', ``inward fault jump'', ``fault alternation'', ``cut-back'', ``hourglass shape median valley'' and ``corrugations and mullion structures''. I describe and compare models focusing on these features.
...
The location of a fault trace is delineated by black \annote[EC]{bold}{Unless there is a thinner dashed line, this wouldn't be needed.} \change[EC]{dash}{dashed} line in Figure~\hyperref[fig_Results1_1]{\ref{fig_Results1_1}} and marked by black arrwos in Figure~\hyperref[fig_Results1_2]{\ref{fig_Results1_2}}.
...
The depressed narrow region inside the median valley is termed as ``trough''\note[EC]{In my opinion, "axial trough" would be better}. Its trace in the map view evolves from a straight line parallel to the ridge-axis (Figure~\hyperref[fig_Results1_4]{\ref{fig_Results1_4}.a}) to a line oblique to the ridge-axis (Figure~\hyperref[fig_Results1_4]{\ref{fig_Results1_4}.b,c,d}). Initially, the trough corresponds to the fault trace, which is almost straight and parallel to the ridge axis. As the normal fault rotates to a lower dip on the lower M side, the trough is no longer coincident with the fault trace. %Also, the trough at the lower M side is pulled to the conjugate plate to the negative $x$-axis direction. 
