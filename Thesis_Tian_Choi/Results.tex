...
Currently, we have run \annote[XT]{hundreds of}{find out exactly number that will be used in 2D model results conclusion} Psedudo-2D models for initial setup and benchmarking with previous studies \remove[EC]{(e.g. }\citep[e.g.,][]{Buck2005, Tucholke2008}\remove[EC]{)}. Based on those \change[EC]{P}{p}seudo-2D models, we further ran 11$+$1 \add[EC]{3D models} (11 models with M \change[EC]{variation}{varying} along the ridge-axis and 1 model with constant M$=0.8$ \remove[EC]{along the ridge-axis}) \remove[EC]{3D models}. The complete list of 3D models is given \remove[EC]{shown} in Table~\hyperref[Tab1_1]{\ref{Tab1_1}}. 

\change[EC]{The available data points are limited due to the huge computation expenses for 3D models}{The numerical cost of a 3D model is non-trivial.} For \remove[EC]{each model to be run to} 2\add[EC]{ }Myr \add[EC]{of model time, each model} usually \change[EC]{needs}{runs on} 192 cores for about \change[EC]{2 days}{48 hours} (\add[EC]{i.e.,} around \change[EC]{10000}{10k or 10$^{4}$} \change[EC]{SUs}{core-hours}). %\add[XT]{use 96 cores can improve the efficiency a little bit (Longer time but smaller amount of SUs needed).})

\change[EC]{Based on the available data points as shown in the tables}{Under this constraint of numerical cost}, \change[EC]{we are able to fix other parameters and only compare the model results with respect to}{I decided to control only the following three parameters while fixing all the others}: 1) \change[EC]{Variation of the functional form}{the functional forms of M variation}; 2) \change[EC]{Variation of the range of M}{the range of M variation}; 3) \change[EC]{Influence of weakening rate}{the rate of strain weakening}. 

%\subsubsection{\change[EC]{Variation of the functional form}{Functional forms of M variation}}
How magma supply varies along the MORs ridge-axis \add[EC]{has not been directly observed and thus} remains \add[EC]{as} \change{a current}{a topic of active} research \remove[EC]{question} \remove[EC]{and we do not have direct quantitative observation over it}. However, \remove[EC]{from }indirect geophysical studies \change[EC]{(e.g.}{using} gravity \change[EC]{,}{and} seismology\remove[EC]{), people} suggest a general \remove[EC]{qualitative} pattern \change[EC]{of}{that} \add[EC]{at} 20 to 50 km\add[EC]{-}long second-order ridge segment\add[EC]{s,} \change[EC]{with magma mostly supply}{magma supply is most abundant(?)} at the segment center and decreases to the end\add[EC]{s}\note[EC]{how about 'tips'?} \citep{Carbotte2015}. Since we \add[EC]{still} do not know exatly how magma supply varies along the ridge, we try three functional forms \remove[EC]{(}linear, sinusoidal and square root\remove[EC]{)}\note[EC]{Don't use parentheses too often! They make things look unimportant but you often put an important piece of information in them!} \remove[EC]{for its variation along the ridge-axis}.
...
\subsubsection{\change[EC]{Comparing M28 in terms of linear, sinusoidal and square root}{Effects of the functional forms of M variation}}
