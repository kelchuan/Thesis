...
\subsubsection{M varies along the ridge axis\note[EC]{Need a more specific, relevant title}} %\note[EC]{If you take the variable-M model as a reference, you don't need to make this subsubsection. }}

\begin{figure}[hc]
  \centering
    \includegraphics[width=1.0\textwidth]{fig_Results1_1.png}
  \caption{\add[EC]{Plastic strain and surface topography of the reference model one in which} M linearly increases from 0.2 to 0.8 \annote[EC]{from front to back}{Add labels, front and back, or mark the corresponding M values on Fig. 8a.}. The top layer is the \add[EC]{five times exaggerated} topography of the model\remove[EC]{ with five times vertical exaggeration}. %The color scale within Figure~\ref{fig_Results1_1}.a is for the topography. Initial seafloor is marked as a reference of zero km in height.  The green, yellow and red colors in the background of blue model domain are plastic strain. Its color scale is shown in Figure~\ref{fig_Results1_1}.e. The number with kyr as a unit beneath each model result is the time for the model with a unit of thousands of year.
The two insets in Fig~\ref{fig_Results1_1}b and Fig.~\ref{fig_Results1_1}d \change[EC]{is for stress}{shows} $\sigma_{xx}$ %(Sxx in the figure) %Positive value (pink and red) means tension and negative (blue) is compression. 
and the inset in Fig.~\ref{fig_Results1_1}h shows $\sigma_{xz}$ with the same color scale with $\sigma_{xx}$.\note[EC]{But the insets show other colors than blue and red!! It's confusing.} The inset in Fig.~\ref{fig_Results1_1}f is a transparent view of plastic strain. The inset in Fig.~\ref{fig_Results1_1}g shows both plastic strain and the velocity vector. %Indicated by the velocity vector, the hanging wall of the detachment fault at low M region (M0.2$\sim$M0.5) is moving in an opposite direction to the hanging wall at higher M region (M$>0.5$).} %\note[XT]{one thing to be noted is that the $dt=0.5yr$ in these series of 3D models, thus I divided the time step by two to get the time (kyr). This figure needs to be revised that the plastic strain scale is actually changing for different time, a way to revise it is to maintain a constant color scale or attach a color scale for each time. Also, it seems a little bit small and two rows is not as preferable as one row or one column to better express the concept of linear time series evolution.}
\note[EC]{You might want to find a more economical way of explaining all the insets...}
}
 \label{fig_Results1_1}
\end{figure}   

\note[EC]{Instead of the commented-out paragraph which seems to be a misplaced overview, remind readers of what the reference model is. For instance, you only mention linearly varying M from 0.2 to 0.8 but what about strain weakening rate?}
...
\change[EC]{in}{For} the first 7.5kyr, %high angle ($\sim 60 \degree$)(consistent with  Anderson's theory of faulting mechanics for a frictional angle of 30$\degree$) 
normal faults\change[EC]{ (shown as higher plastic strain shear bands}{, represented by localized plastic strain,} %with a thickness of 2$\sim$4 times of the width of a single hexahedron element) 
begin to form near the ridge axis (Fig.~\ref{fig_Results1_1}a). \remove[EC]{in terms of plastic strain localization near the ridge center (weakest place to deform),} \remove[EC]{because of the thickness of the crust is thinnest at the ridge center}\remove[EC]{due to our thermal structure setup}. \change[EC]{For each timestep, the tensional stress}{Stresses due to plate motions} accumulate faster at the lower M side \remove[EC]{where the crust reaches a yielding point earlier} than \add[EC]{at} higher M side and so the fault first initiate at the \remove[EC]{front (}lower M side and then propagates to the \remove[EC]{back (}higher M side. 
%However, the along ridge-axis coupling (internal strength preventing relative displacement (i.e. rotation, offset) between two neighbors along the Z-axis) assists in fault propagation from front to back and reduces the time difference in initiation of faulting along the ridge-axis \annote[XT]{when comparing with separate 2D models}{It probably will be verified after a 2D results analysis and conclusion}.  

At 52.5kyr (Figure~\ref{fig_Results1_1}.b), the normal fault on the right hand side of the ridge axis continues to evolve while the one on the left becomes inactive. The choice of which fault will delvelop \change[EC]{is a random event}{seems to be made due to a small differences between the two faults seen in} Fig.~\ref{fig_Results1_1}a \change[EC]{since}{although} the model setup is symmetrical across the ridge-axis. 

At 240kyr (Figure~\ref{fig_Results1_1}.c), the median valley further deepens and widens. The \annote[EC]{detachment}{you are mentioning this term out of the blue. First say when and how it forms.} keeps active and \annote[EC]{extends to $\sim$18km in length horizontally (longest at M$=0.2$)}{the meaning is unclear.} with the dip angle decreases from \annote[EC]{initially}{do you know the difference between an adverb and an adjective?} $\sim$60$\degree$ to 30$\degree$\annote[EC]{(at the root of the fault)}{you seem to use parentheses too much.} and 0$\degree$ where the fault interface is exposed to the seafloor. However, for the detachment at the higher M side (especially the last three elements along the Z-axis), the dip angle remains high. The maximum relief between highest point in breakaway and lowest point in the median valley becomes larger than 1km.
