...
\add[XT]{Preliminary pseudo-2D results show that the model behavior in faulting pattern is sensitive to the weakening rate. Two kinds of weakening rate will be tested in 3D models. They are type one: cohesion linearly decreases from 44Mpa to 4Mpa for plastic strain accumulating from 0 to 0.1; type two: cohesion linearly decreases from 44Mpa to 4Mpa for plastic strain accumulating from 0 to 0.33. In addition, we will focus on experimenting on different patterns of M variation in terms of the functional forms (e.g. discrete increment, linear, sinusuidal and square root) as well as its wavelength (e.g. 10km, 20km and 40km) and the ranges of M (e.g. 0.2$\sim$0.8, 0.5$\sim$0.8 and 0.5$\sim$0.7)}
